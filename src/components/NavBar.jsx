  ...
  return (
    <nav className="w-full flex py-6 justify-between items-center navbar">
      ...
      <div className="sm:hidden flex flex-1 justify-end items-center">
        <img src={toogle ? close : menu} alt="menu" className='w-[28px] h-[28px] object-container' onClick={() => setToogle((prev) => !prev)}/>
        ...
        <div className={`${toogle ? 'flex' : 'hidden'} p-6 bg-black-gradient absolute top-20 right-0 mx-4 my-2 min-w-[140px] rounded-xl sidebar`}>
          <ul className="list-none flex flex-col justify-start">
            {navLinks.map((nav, index) => (
              <li key={nav.id} className={`font-poppins font-normal cursor-pointer text-[16px] text-white ${index === navLinks.length -1 ? 'mb-0' : 'mb-3'} `}>
                <a href={`#${nav.id}`}>
                  {nav.title}
                </a>
              </li>
